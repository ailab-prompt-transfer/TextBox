...
class PTG(Pretrained_Models):
    ...
    def sentence_embedding(self, text):
        encoding_dict = self.bert_tokenizer(text, max_length=self.bert_tokenizer.model_max_length, padding=True, truncation=True, return_tensors="pt")
    ...
    def MHA(self, query, key, value):
        ...
        # b*h, 1, d
        query = query.reshape(batch_size, -1, self.head_num, self.head_dim).transpose(1, 2).reshape(batch_size * self.head_num, -1, self.head_dim)
        # b*h, tn, d
        key = key.reshape(batch_size, -1, self.head_num, self.head_dim).transpose(1, 2).reshape(batch_size * self.head_num, -1, self.head_dim)
        # b*h, tn, pl*d
        value = value.reshape(batch_size, self.task_num, -1, self.head_num, self.head_dim).permute(0, 3, 1, 2, 4).reshape(batch_size * self.head_num, self.task_num, -1)
