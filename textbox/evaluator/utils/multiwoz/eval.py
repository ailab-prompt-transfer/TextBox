...
class MultiWozEvaluator(object):
    ...
    def wrap_evaluation_result(self, result_list):
        field = ["dial_id", "turn_num", "user", "bspn_gen", "bsdx", "resp_gen", "resp", "aspn_gen", "aspn", "dspn_gen", "dspn", "bspn", "pointer"]
    ...
    def validation_metric(self, data):
        ...
        success, match, req_offer_counts, dial_num = self.context_to_response_eval(data, same_eval_as_cambridge=self.cfg.same_eval_as_cambridge)
    ...
    def _get_metric_results(self, data, domain="all", file_list=None):
        ...
        success, match, req_offer_counts, dial_num = self.context_to_response_eval(data, file_list, same_eval_as_cambridge=cfg.same_eval_as_cambridge)
        req_slots_acc = {}
        ...
        for dial_id in dials:
            ...
            for turn_num, turn in enumerate(dial):
                ...
                if self.cfg.enable_dspn:
                    ...
                    turn_dom_bs = []
                    if self.cfg.enable_bspn and not self.cfg.use_true_bspn_for_ctr_eval and (cfg.bspn_mode == "bspn" or self.cfg.enable_dst):
    ...
    def dialog_state_tracking_eval(self, data, eval_dial_list=None, bspn_mode="bspn", no_name=False, no_book=False):
        ...
        for dial_id in dials:
            ...
            for turn_num, turn in enumerate(dial):
                if turn_num == 0:
                    continue
                gen_cons = self._bspn_to_dict(turn[bspn_mode + "_gen"], no_name=no_name, no_book=no_book, bspn_mode=bspn_mode)
                ...
                if eval_dial_list is None:
                    tp, fp, fn, acc, false_slots = self._constraint_compare(truth_cons, gen_cons, slot_appear_num, slot_correct_num)
                else:
    ...
    def context_to_response_eval(self, data, eval_dial_list=None, same_eval_as_cambridge=False):
        ...
        for dial_id in dials:
            ...
            # print('\n',dial_id)
            success, match, stats, counts = self._evaluateGeneratedDialogue(dial, goal, reqs, counts, same_eval_as_cambridge=same_eval_as_cambridge)
    ...
    def _evaluateGeneratedDialogue(self, dialog, goal, real_requestables, counts, soft_acc=False, same_eval_as_cambridge=False):
