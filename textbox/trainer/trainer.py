...
from .scheduler import AbstractScheduler, InverseSquareRootScheduler, CosineScheduler, LinearScheduler, ConstantScheduler
from torch.utils.data import DataLoader
...
class Trainer(AbstractTrainer):
    ...
    def _build_optimizer(self, optimizer: str, scheduler: Optional[str]) -> Union[optim.Optimizer, AbstractScheduler]:
    ...
    def _train_epoch(
        ...
        if not self.disable_tqdm:
            ...
            train_tqdm = tqdm(range(train_data_len), desc=f"train {epoch_idx:4}", dynamic_ncols=True, postfix={"loss": None}, unit="step")
    ...
    def _valid(
        ...
        with self._summary_tracker.new_epoch("valid"):
            if "loss" in self.metrics_for_best_model:
                self.model.eval()
                if not self.disable_tqdm:
                    valid_tqdm = tqdm(valid_data, desc=f"valid {self.timestamp.valid_epoch:4}", dynamic_ncols=True, postfix={"loss": None}, unit="step")
                else:
    ...
    def resume_checkpoint(self, resume_dir: str):
        ...
        if checkpoint["config"]["optimizer"].lower() != self.config["optimizer"]:
            self.logger.warning(
                "Optimizer configuration given in config file is different from that of checkpoint. " "This may yield an exception while state_dict is being loaded."
            )
    ...
    def fit(
        ...
        self.logger.info(f"====== Finished training, best validation result " f"at train epoch {self.best_valid_timestamp.train_epoch} ======")
